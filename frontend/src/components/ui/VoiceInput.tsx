  ...
  return (
    <div className={`relative ${className}`}>
      <button
        ...
        className={`w-14 h-14 rounded-full flex items-center justify-center transition-all duration-300 shadow-lg border-4 border-white ${
          isListening 
            ? 'bg-red-500 animate-pulse shadow-red-300/50 scale-110' 
            : 'bg-orange-500 hover:bg-orange-600 shadow-orange-300/50 hover:scale-105 active:scale-95'
        }`}
      >
        <svg 
          width="24" 
      ...
      {isListening && (
        <div className="absolute -top-10 left-1/2 transform -translate-x-1/2">
          <span className="bg-black text-white px-3 py-1 rounded-full text-xs whitespace-nowrap">
            Listening...
          </span>
        </div>
      )}
