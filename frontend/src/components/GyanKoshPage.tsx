...
const GyanKoshPage = ({ onBack }: GyanKoshPageProps) => {
  ...
  const { isListening, startListening, stopListening, isSupported, transcript: liveTranscript } = useSpeechRecognition(handleVoiceResult)
    ...
    switch (data.type) {
      ...
      case 'input_transcription':
        ...
        if (data.text && data.text.trim()) {
          if (fullResponseRef.current && data.text === fullResponseRef.current) {
            console.log('⚠️ Ignoring bot response echo in input_transcription')
            break
          }
  ...
  // Setup continuous listening and audio streaming
  useEffect(() => {
    const setupConnection = async () => {
      try {
        setIsConnecting(true)
        
        // Initialize backend connection and SSE stream
        await apiService.connect()
        // Use stable wrapper that calls the ref
        apiService.startEventStream((data) => streamHandlerRef.current(data))
        
        // Only setup voice/audio for voice mode
        if (viewMode === 'voice') {
          // Check browser support
          if (!navigator?.mediaDevices?.getUserMedia) {
            console.warn('❌ MediaDevices API not supported')
            setPermissionDenied(true)
            setIsConnected(true) // Still mark as connected for chat
            return
          }
          ...
          audioService.setUserSpeakingCallback((isSpeaking: boolean) => {
            if (isSpeaking) {
              ...
              setIsBotSpeaking(false) // Interrupt bot when user speaks
            } else {
        ...
        if (viewMode === 'voice' && error instanceof DOMException && error.name === 'NotAllowedError') {
  ...
  const switchToChat = () => {
    ...
    setViewMode('chat')
    console.log('✅ Chat mode active - voice and audio stopped')
  }
