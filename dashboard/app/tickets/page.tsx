...
import type { TicketApiItem } from "@/types/api";
...
function normalizeSeverity(value: string | null | undefined): SeverityType {
...
function buildSlaTags(status: StatusType, createdAt: string | null): string[] {
  const tags = [`Opened ${formatDate(createdAt)}`];
  tags.push(status === "resolved" ? "Resolved" : "Pending");
  return tags;
}

function mapTicketFromApi(ticket: TicketApiItem): Ticket {
  ...
  return {
    ...
    area: formatLocation(ticket.location_lat, ticket.location_lng),
    sla: buildSlaTags(status, ticket.created_at),
    assignee: "Unassigned",
    status,
    sessionCount: ticket.conversation_events?.length ?? 0,
  };
}
...
function truncateId(id: string, length = 20) {
    ...
    <div className="relative rounded-[8px] shrink-0 w-full">
      <div className="content-stretch flex gap-[8px] items-start px-[12px] py-[6px] relative w-full">
        <div className="basis-0 content-stretch flex grow items-start justify-between min-h-px min-w-px relative self-stretch shrink-0">
          {/* Ticket ID */}
          <div className="content-stretch flex items-start max-w-[220px] relative shrink-0 w-[200px]">
          ...
          {/* Title */}
          <div className="content-stretch flex flex-col items-start max-w-[220px] relative shrink-0 w-[200px]">
    ...
    <div className="relative rounded-[8px] shrink-0 w-full">
      <div className="content-stretch flex gap-[8px] items-start px-[12px] py-[6px] relative w-full">
        <div className="basis-0 content-stretch flex grow items-start justify-between min-h-px min-w-px relative self-stretch shrink-0">
          ...
          {/* Title */}
          <div className="content-stretch flex flex-col items-start max-w-[220px] relative shrink-0 w-[200px]">
            <p className="font-['Switzer_Variable:Regular',sans-serif] leading-[1.4] not-italic relative shrink-0 text-[#e9e8e8] text-[13px] tracking-[-0.24px] w-full truncate">
              {ticket.title}
            </p>
          </div>
